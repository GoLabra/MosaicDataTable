...
export const MosaicDataTableHeadCore = <T,>(props: EnhancedTableProps<T>) => {
    ...
    const getRow = useCallback((params: { children?: ReactNode }) => {
        ...
        for (const plugin of rowRenderPlugins) {
            var row = plugin.renderHeadRow?.(props.gridApi, cellStyle, params.children);
            if (row) {
                return row;
            }
        }

        return (<TableRow key="head-row" sx={cellStyle} >{params.children}</TableRow>);
    }, [rowRenderPlugins]);
    ...
    const getRowStyle = useCallback((): SxProps<Theme> => {
        return rowStylePlugins.reduce((acc: SxProps<Theme>, plugin: MosaicDataTableHeadRowStylePlugin) => {
            const rowStyle = plugin.getHeadRowStyle?.(props.gridApi);
            return {
                ...acc,
                ...rowStyle
            }
    ...
    const getCell = useCallback((params: { headCell: ColumnDef<T>, children?: ReactNode }) => {
        ...
        for (const plugin of cellRenderPlugins) {
            var cell = plugin.renderHeadCell?.(params.headCell, props.gridApi, cellStyle, params.children);
            if (cell) {
                return cell;
            }
        }
